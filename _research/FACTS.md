...
- **The graph is the MAP the loop reads. The journal is the MEMORY it writes.** The heartbeat reads pose state + journal and writes intent; the walker reads intent + the graph and writes pose state. Single-writer per file, no races. All writes atomic (tmp + os.replace). [LP-HEARTBEAT]
- **UPDATED 2026-08-11 (v0.4.0):** the clause "the graph is read-only at runtime — not something experience accretes into" was true when written and is now only half true. The *asset* graph is still built offline. But each walker now writes a LIVED record beside it (`runtime/lived.py`): visits, first and last time a pose was stood in, dwell, the mood-band brought, and which clips have actually rolled. 76 days of it were recovered from the walker's own pick-log — **847,659 picks**. So experience does accrete now; it accretes in a sidecar keyed to the graph rather than inside the file the generator regenerates every 20 minutes. Say "the graph is the map, the journal is the memory, and the lived record is the wear on the carpet." [LP-HEARTBEAT]
- The graph also carries TIME now (`runtime/graph_provenance.py`): every node and edge knows when the system learned it exists, and a clip that disappears is tombstoned rather than forgotten. On the production host that is 259/259 nodes and 1472/1472 edges. [LP-HEARTBEAT]
...
- **The cost of that guarantee, measured (2026-08-11):** the IC token went 401 on 2026-07-14 and nobody noticed for **27 days**. Zero prompts carried the clause; every tick looked perfectly healthy. The design is right and the lesson is the pair to it: **fail-soft without a detector is just silence.** There is now a `world_context` check in `health/` that fails when the phrase is empty. If the fail-soft property comes up on stage, tell both halves. [LP-CONTEXT]
...
- **Honest note on "typed" in THIS system (2026-08-11):** until v0.4.0 the edge type system was one bit — `idle` or `transition`, both meaning "a playable clip" — and every claim about typed edges carrying relational semantics was describing Zep, not this. Since v0.4.0 each edge also carries a **manner and a valence** derived from its own motion prompt (`runtime/edge_style.py`): 1,030 of 1,472 real edges typed, 90.3% of transitions, across ten manners, and the type does real work — a weary character prefers the clip that sags over the clip that lunges. 442 edges remain untyped and are reported untyped rather than quietly called neutral. If asked, say that: the typing is real now, and it is 70% coverage, not 100%. [RAG-CONTRAST]
- **The measured version of the whole contrast:** removing the graph-hop relevance term changes what gets retrieved in **40 of 40 real replays** across both characters, sharing only 5-10% of its picks with a graph-blind reader. That is the one number that answers "why does this project have a graph" without a story. Scorecard + method: `tests/test_context_graph_probe.py`. [RAG-CONTRAST]
